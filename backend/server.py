...
async def initialize_database(secret_key: str):
    """
    Initialize database with admin user. 
    Only works if no users exist and correct secret is provided.
    """
    ...
    # Create admin user
    admin_password = os.environ.get("ADMIN_PASSWORD", "MyPropOps@Admin2026!")
    ...
    await db.memberships.create_index([("user_id", 1), ("org_id", 1)])
    await db.properties.create_index("org_id")
    await db.tenants.create_index("org_id")
    ...
    return {
        ...
        "note": "You can now login with the admin account"
    }
...
# Create uploads directory for esign documents
ESIGN_UPLOAD_DIR = ROOT_DIR / "uploads" / "esign"
ESIGN_UPLOAD_DIR.mkdir(parents=True, exist_ok=True)
...
async def get_esign_templates(user = Depends(get_current_user)):
    """Get eSignature templates for the organization"""
    membership = await db.memberships.find_one({"user_id": user['id']})
    if not membership:
        return []
    
    templates = await db.esign_templates.find({"org_id": membership['org_id']}).sort("created_at", -1).to_list(100)
    return [{
        "id": t.get('id'),
        "name": t.get('name'),
...
async def upload_esign_template(
    file: UploadFile = File(...),
    ...
    file_ext = Path(file.filename).suffix
    file_path = f"templates/{membership['org_id']}/{template_id}{file_ext}"
    full_path = ESIGN_UPLOAD_DIR / file_path
    ...
    return {"id": template_id, "name": template_name, "status": "uploaded"}

@api_router.delete("/esign/templates/{template_id}")
async def delete_esign_template(template_id: str, user = Depends(get_current_user)):
    """Delete a document template"""
    membership = await db.memberships.find_one({"user_id": user['id']})
    if not membership:
        raise HTTPException(status_code=403, detail="Not authorized")
    
    template = await db.esign_templates.find_one({"id": template_id, "org_id": membership['org_id']})
    if not template:
        raise HTTPException(status_code=404, detail="Template not found")
    
    # Delete file
    file_path = ESIGN_UPLOAD_DIR / template['file_path']
    if file_path.exists():
        file_path.unlink()
...
async def get_esign_documents(user = Depends(get_current_user)):
    """Get sent documents for signature"""
    membership = await db.memberships.find_one({"user_id": user['id']})
    if not membership:
        return []
    
    documents = await db.esign_documents.find({"org_id": membership['org_id']}).sort("created_at", -1).to_list(100)
    return [{
        "id": d.get('id'),
        ...
        "status": d.get('status'),
        "signing_token": d.get('signing_token'),
        "signed_document_url": f"/api/esign/download/{d.get('id')}" if d.get('status') == 'signed' else None,
        "created_at": d.get('created_at'),
        "signed_at": d.get('signed_at')
    } for d in documents]

@api_router.post("/esign/send")
async def send_document_for_signature(data: ESignSendRequest, user = Depends(get_current_user)):
    """Send a document for signature"""
    membership = await db.memberships.find_one({"user_id": user['id']})
    if not membership:
        raise HTTPException(status_code=403, detail="Not authorized")
    
    template = await db.esign_templates.find_one({"id": data.template_id, "org_id": membership['org_id']})
    if not template:
        raise HTTPException(status_code=404, detail="Template not found")
    ...
    template_path = ESIGN_UPLOAD_DIR / template['file_path']
    doc_path = f"documents/{membership['org_id']}/{document_id}.pdf"
    full_doc_path = ESIGN_UPLOAD_DIR / doc_path
    full_doc_path.parent.mkdir(parents=True, exist_ok=True)
    shutil.copy(template_path, full_doc_path)
    ...
    return {
        "id": document_id,
        "signing_token": signing_token,
        "signing_url": f"/sign/{signing_token}",
        "status": "sent"
    }

@api_router.get("/esign/document/{token}")
async def get_document_for_signing(token: str):
    """Get document details for signing (public endpoint)"""
    document = await db.esign_documents.find_one({"signing_token": token})
    if not document:
        raise HTTPException(status_code=404, detail="Document not found")
    ...
    expires_at = datetime.fromisoformat(document.get('expires_at', datetime.now(timezone.utc).isoformat()))
    ...
    return {
        ...
        "signer_email": document.get('signer_email'),
        "sender_name": document.get('sender_name'),
        "organization_name": document.get('organization_name'),
        "preview_url": f"/api/esign/preview/{token}",
        "document_url": f"/api/esign/preview/{token}",
        "created_at": document.get('created_at')
    }

@api_router.get("/esign/preview/{token}")
async def preview_document(token: str):
    """Preview document PDF (public endpoint)"""
    document = await db.esign_documents.find_one({"signing_token": token})
    if not document:
        raise HTTPException(status_code=404, detail="Document not found")
    
    file_path = ESIGN_UPLOAD_DIR / document['document_path']
    if not file_path.exists():
        raise HTTPException(status_code=404, detail="Document file not found")
    
    return FileResponse(file_path, media_type="application/pdf")

@api_router.post("/esign/sign/{token}")
async def sign_document(token: str, data: ESignSignRequest):
    """Sign a document with signature image"""
    ...
    try:
        sig_data = data.signature_image.split(',')[1] if ',' in data.signature_image else data.signature_image
    ...
    sig_path = ESIGN_UPLOAD_DIR / f"temp_sig_{document['id']}.png"
    sig_image.save(sig_path, 'PNG')
    
    # Create signature overlay PDF
    overlay_path = ESIGN_UPLOAD_DIR / f"temp_overlay_{document['id']}.pdf"
    c = canvas.Canvas(str(overlay_path), pagesize=letter)
    
    # Add signature at bottom of page
    sig_width = 200
    sig_height = 60
    x_pos = 72  # 1 inch from left
    y_pos = 100  # Near bottom
    
    c.drawImage(str(sig_path), x_pos, y_pos, width=sig_width, height=sig_height, preserveAspectRatio=True)
    
    # Add signature details text
    c.setFont("Helvetica", 8)
    c.drawString(x_pos, y_pos - 15, f"Signed by: {data.signer_name}")
    c.drawString(x_pos, y_pos - 25, f"Date: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}")
    c.drawString(x_pos, y_pos - 35, f"IP-verified electronic signature")
    
    c.save()
    
    # Merge signature overlay with original document
    original_path = ESIGN_UPLOAD_DIR / document['document_path']
    signed_path = ESIGN_UPLOAD_DIR / f"signed/{document['org_id']}/{document['id']}_signed.pdf"
    ...
    for i, page in enumerate(original_pdf.pages):
        if i == len(original_pdf.pages) - 1:  # Last page
    ...
    await db.esign_documents.update_one(
        ...
        {"$set": {
            ...
            "signed_document_path": f"signed/{document['org_id']}/{document['id']}_signed.pdf",
            "signer_name_confirmed": data.signer_name,
            "signer_email_confirmed": data.signer_email
        }}
    )
    ...
    await create_notification(
        ...
        document['sender_id'],
        "Document Signed",
        f"{data.signer_name} has signed '{document['document_name']}'",
        "high",
        "document_signed",
    ...
    return {
        ...
        "signed_document_url": f"/api/esign/download/{document['id']}"
    }

@api_router.get("/esign/download/{document_id}")
async def download_signed_document(document_id: str):
    """Download signed document"""
    document = await db.esign_documents.find_one({"id": document_id})
    if not document:
        raise HTTPException(status_code=404, detail="Document not found")
    ...
    file_path = ESIGN_UPLOAD_DIR / document.get('signed_document_path', '')
    if not file_path.exists():
    ...
    return FileResponse(
        file_path, 
        media_type="application/pdf",
        filename=f"{document['document_name']}_signed.pdf"
    )

@api_router.post("/esign/documents/{document_id}/resend")
async def resend_signature_request(document_id: str, user = Depends(get_current_user)):
    """Resend signature request notification"""
    membership = await db.memberships.find_one({"user_id": user['id']})
    if not membership:
        raise HTTPException(status_code=403, detail="Not authorized")
    
    document = await db.esign_documents.find_one({"id": document_id, "org_id": membership['org_id']})
    if not document:
        raise HTTPException(status_code=404, detail="Document not found")
    ...
    return {"status": "sent", "message": "Reminder sent"}
